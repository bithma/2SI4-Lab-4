...
void testCopyBoundary() {

	int row=2,col=2;
    int data[2][2] = {{0,4,}, {0,0}};
	int*input_data[2];
	for (int i=0;i<2;i++){
	    	input_data[i]=data[i];
	}
...
void testAddBoundary() {
	   int add_to_data[2][2] = {{0,0}, {0,0}}; // add to 'data' matrix, dimension not match
	   int add_to_above[2][2] = {{0,0}, {0,0}}; // add to 'add_to_data' matrix
	   int row=2;int col=2;
...
void testSubMatrixBoundry() {

    int subinvalid_row = 0, subinvalid_col = 0;
...
void stringboundary() {
	   int add_to_data[2][2] = {{1,1}, {1,1}}; // add to 'data' matrix, dimension not match
	   int add_to_above[2][2] = {{1,1}, {1,1}}; // add to 'add_to_data' matrix
	   int row=2;int col=2;
...
bool runAllTests(int argc, char const *argv[]) {
	...
	s.push_back(CUTE(testGetElementBoundary));
	s.push_back(CUTE(testSetElementSame));
	s.push_back(CUTE(testCopyBoundary));


	s.push_back(CUTE(testAddBoundary));
	s.push_back(CUTE(testSubMatrixBoundry));
	s.push_back(CUTE(testUpperBoundary));
	s.push_back(CUTE(stringboundary));
